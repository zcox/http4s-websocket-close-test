...
class App[F[_]](log: Logger[F])(implicit F: Effect[F]) extends StreamApp[F] with Http4sDsl[F] {
  ...
  def service(topic: Topic[F, Option[String]]) = HttpService[F] {
    case GET -> Root / "messages" => 
      val send = Stream.eval(log.info("Subscriber started")).drain ++ topic.subscribe(100).tail.unNone.map(s => Text(s)).onFinalize(log.info("Subscriber closed"))
      ...
      log.info("Evaluated response effect") *> WebSocketBuilder[F].build(send, receive, onNonWebSocketRequest = onNonWebSocketRequest, onHandshakeFailure = onHandshakeFailure)
  }
